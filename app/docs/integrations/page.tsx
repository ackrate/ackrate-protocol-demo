...
export default function IntegrationsDocs() {
  return <><p className="eyebrow">Docs / Integrations · alpha</p><h1>Connect a paid service.</h1>
    ...
    <h2>Hosted gateway status</h2><p>The Express companion is deployed at <a href="https://reapp.live">reapp.live</a>. On September 25, 2026, its session endpoint responded, but starting a workspace failed with an unfunded Mainnet account error. It is not currently a working hosted quick start. Use the local Testnet starter while funding and runtime configuration are repaired.</p>
    <p>Vercel staging serves the app and docs; it does not host the companion’s persistent in-memory sessions. The <Link href="/express">Express demo</Link> retains historical August Mainnet receipts, not evidence of current gateway availability. Wallet sign-in and payments also require deployment readiness.</p>
