...
export default function CliDocs() {
  return <><p className="eyebrow">Docs / CLI</p><h1>One CLI for the protocol.</h1>
    ...
    <h2>Install and inspect</h2><CodeBlock>{`npm install -g @ackrate/cli@0.2.1
ackrate --help
ackrate demo`}</CodeBlock><p>With no target, <code>ackrate demo</code> lists the available reference flow without submitting a payment.</p>
    <h2>Configure Mainnet</h2><CodeBlock>{`ackrate init --network mainnet --user-signer USER_IDENTITY --agent-signer AGENT_IDENTITY --merchant G_ADDRESS --budget 3 --price 1
ackrate setup`}</CodeBlock><p>Replace the identity and address placeholders. <code>setup</code> performs read-only readiness checks on Mainnet. The user and agent need funded Stellar identities; the user also needs Circle USDC.</p>
    <h2>Authorize and pay</h2><CodeBlock>{`ackrate mandate create --budget 3 --expiry 3600 --confirm-real-usdc
ackrate pay 1 --confirm-real-usdc`}</CodeBlock><p>These commands authorize and spend real USDC. Review the recipient and amounts before running them. The confirmation flag records explicit consent.</p>
    <h2>Recover an interrupted payment</h2><CodeBlock>ackrate settlement reconcile</CodeBlock><p>Reconcile before another payment. Acknowledge a recorded successful settlement only with its exact transaction hash. Never delete pending state to force a retry.</p>
