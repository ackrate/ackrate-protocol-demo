...
export default function MerchantsPage() {
  ...
  return (
    ...
    <motion.div aria-hidden className="fixed inset-x-0 top-0 z-[70] h-0.5 origin-left bg-gradient-to-r from-neutral-400 via-neutral-300 to-neutral-300" style={{ scaleX: progress }} />
    <main className="relative mx-auto w-full max-w-7xl overflow-hidden px-4 pb-20 pt-8 sm:px-6">
      ...
      <motion.header {...fade()} className="pt-7">
        <div className="inline-flex items-center gap-2 rounded-full border border-neutral-300/15 bg-neutral-400/[0.06] px-3.5 py-1.5 text-[11px] font-semibold tracking-[0.17em] text-neutral-200/90">
          <span className="h-1.5 w-1.5 rounded-full bg-neutral-400 shadow-[0_0_12px_rgba(255,255,255,.9)]" />
        ...
        <h1 className="mt-6 max-w-5xl text-4xl font-black leading-[1.02] tracking-[-0.045em] text-white sm:text-6xl lg:text-7xl">
          ...
          <span className="bg-gradient-to-r from-neutral-300 via-neutral-200 to-neutral-300 bg-clip-text text-transparent">
      ...
      <motion.section {...fade(0.06)} className="mt-10 grid overflow-hidden rounded-2xl border border-white/10 bg-white/[0.025] sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={label} className="border-b border-white/10 p-5 last:border-0 sm:border-r lg:border-b-0">
            <strong className="text-3xl font-black tracking-tight text-neutral-200">{value}</strong>
            <span className="mt-1 block text-xs text-white/45">{label}</span>
          </div>
        ))}
      </motion.section>

      <motion.section {...fade(0.07)} className="relative mt-8 overflow-hidden rounded-3xl border border-neutral-300/15 bg-[radial-gradient(circle_at_20%_20%,rgba(255,255,255,.12),transparent_38%),#050a08] p-5 sm:p-8">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,.025)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.025)_1px,transparent_1px)] bg-[size:38px_38px]" aria-hidden />
        <div className="relative grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Complete evidence map</p>
            ...
            <div className="relative mt-6 h-64 overflow-hidden rounded-2xl border border-neutral-300/15 bg-black/25">
              <SecurityProofField />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {requirements.map(({ label, result, copy, href, icon: Icon }, index) => (
              <motion.a
                ...
                className="group relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4 [transform-style:preserve-3d] hover:border-neutral-300/25"
              >
                <motion.div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-neutral-300 to-transparent" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 0.65, delay: 0.15 + index * 0.055 }} />
                <div className="flex items-start justify-between gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-xl border border-neutral-300/15 bg-neutral-400/[0.07] text-neutral-300"><Icon className="h-4 w-4" /></span>
                  <span className="rounded-md border border-neutral-300/15 bg-neutral-400/[0.06] px-2 py-1 text-[9px] font-bold tracking-[0.11em] text-neutral-200">{result}</span>
                </div>
                <h3 className="mt-4 text-sm font-bold text-white/85">{label}</h3>
                <p className="mt-2 text-xs leading-relaxed text-white/40">{copy}</p>
                <span className="mt-4 inline-flex items-center gap-1.5 text-[10px] font-semibold text-neutral-300/65 group-hover:text-neutral-200">Inspect evidence <ArrowUpRight className="h-3 w-3" /></span>
              </motion.a>
            ))}
          </div>
        </div>
      ...
      <motion.section {...fade(0.08)} className="relative mt-8 overflow-hidden rounded-3xl border border-neutral-300/15 bg-[#050a08] p-5 sm:p-8">
        <div className="pointer-events-none absolute inset-0 opacity-40" aria-hidden>
          ...
          <motion.div
            className="absolute inset-x-0 h-px bg-gradient-to-r from-transparent via-neutral-300/70 to-transparent shadow-[0_0_24px_rgba(255,255,255,.55)]"
            animate={{ top: ["8%", "92%", "8%"] }}
            ...
            <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {gateStages.map((stage, index) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-left transition ${index === gateIndex ? "border-neutral-300/35 bg-neutral-400/[0.1]" : "border-white/[0.07] bg-black/20 hover:border-white/15"}`}
                >
                  <span className={`text-[10px] font-semibold tracking-[0.13em] ${index === gateIndex ? "text-neutral-300" : "text-white/30"}`}>{String(index + 1).padStart(2, "0")}</span>
                  <strong className={`mt-1 block text-xs ${index === gateIndex ? "text-neutral-100" : "text-white/55"}`}>{stage.label}</strong>
                </button>
              ))}
            </div>
          </div>

          <div className="relative overflow-hidden rounded-2xl border border-neutral-300/15 bg-black/35 p-5 font-mono sm:p-6">
            <div className="flex items-center justify-between gap-4 border-b border-white/[0.07] pb-4">
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-rose-300/70" /><span className="h-2 w-2 rounded-full bg-amber-200/70" /><span className="h-2 w-2 rounded-full bg-neutral-300/70" />
              </div>
            ...
            <div className="mt-5 min-h-56">
              ...
              <motion.div key={activeGate.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="mt-7">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-xs tracking-[0.14em] text-neutral-300">SCANNING / {activeGate.label.toUpperCase()}</span>
                  <span className="rounded-md border border-neutral-300/20 bg-neutral-400/[0.08] px-2 py-1 text-[10px] font-bold text-neutral-200">PASS</span>
                </div>
              ...
              <div className="mt-7 h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
                <motion.div key={gateIndex} className="h-full rounded-full bg-gradient-to-r from-neutral-500 to-neutral-300" initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 1.55, ease: "linear" }} />
              </div>
            </div>
            <div className="mt-3 flex items-center gap-2 border-t border-white/[0.07] pt-4 text-[10px] text-neutral-300/55">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
        <motion.div {...fade(0.1)} className="glass rounded-2xl p-4 sm:p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Negative-path suite</p>
        ...
        <motion.div {...fade(0.14)} className="relative overflow-hidden rounded-2xl border border-neutral-300/15 bg-[linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.018))] p-5 sm:p-7">
          <div className="absolute -right-20 -top-20 h-52 w-52 rounded-full bg-neutral-400/10 blur-3xl" aria-hidden />
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Published contract evidence</p>
              <h2 className="mt-2 text-2xl font-bold tracking-tight text-white">{selected.label}</h2>
            </div>
            <span className="grid h-12 w-12 place-items-center rounded-xl border border-neutral-300/20 bg-neutral-400/10 text-neutral-300"><SelectedIcon className="h-5 w-5" /></span>
          </div>
          ...
          <div className="relative mt-7 grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-neutral-300/15 bg-neutral-400/[0.045] p-4"><span className="text-[10px] font-semibold uppercase tracking-widest text-neutral-300/60">Contract result</span><p className="mt-2 flex items-center gap-2 text-sm font-semibold text-neutral-100"><X className="h-4 w-4 text-rose-300" /> {selected.outcome}</p></div>
          </div>
          ...
          <div className="relative mt-5 rounded-xl border border-white/[0.08] bg-[#060a08] p-4 font-mono">
            <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-neutral-300/60"><BadgeCheck className="h-4 w-4" /> Exact Rust test</div>
            <code className="mt-3 block break-words text-xs text-neutral-100/80">{selected.test}</code>
            <a href={selected.source} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-neutral-300 transition hover:text-neutral-200">Inspect source and snapshots <ArrowUpRight className="h-3.5 w-3.5" /></a>
          </div>
        </motion.div>
      </section>
      ...
      <motion.section {...fade(0.17)} className="mt-12 overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.02] p-5 sm:p-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Enforcement boundary</p>
          ...
          <a href={`${REPO}/blob/main/docs/security-data-flow.md`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-neutral-300 hover:text-neutral-200">
        ...
        <div className="mt-7 grid gap-3 lg:grid-cols-[1fr_auto_1fr_auto_1.15fr_auto_1fr] lg:items-stretch">
          {[
            ...
            <div key={title} className="contents">
              <div className={`rounded-2xl border p-4 ${index === 2 ? "border-neutral-300/25 bg-neutral-400/[0.08]" : "border-white/[0.07] bg-black/15"}`}>
                <span className={`text-[10px] font-semibold tracking-[0.15em] ${index === 2 ? "text-neutral-300" : "text-white/30"}`}>{eyebrow}</span>
                <strong className="mt-2 block text-sm text-white/85">{title}</strong>
                <p className="mt-2 text-xs leading-relaxed text-white/40">{copy}</p>
              </div>
              {index < 3 ? <ArrowRight className="mx-auto hidden h-5 w-5 self-center text-neutral-300/40 lg:block" /> : null}
            </div>
          ))}
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div key={label} className="rounded-xl border border-neutral-300/10 bg-neutral-400/[0.035] p-4">
              <strong className="text-xs tracking-[0.14em] text-neutral-300">{label}</strong>
      ...
      <motion.section {...fade(0.18)} className="mt-12">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Independent review trail</p><h2 className="mt-2 text-3xl font-bold tracking-tight text-white">Start with the evidence, not the pitch.</h2></div>
          <a href={`${REPO}/actions`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-neutral-300 hover:text-neutral-200">View continuous checks <ArrowUpRight className="h-4 w-4" /></a>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {evidence.map(({ title, copy, href, icon: Icon }) => (
            <a key={title} href={href} target="_blank" rel="noreferrer" className="group rounded-2xl border border-white/[0.08] bg-white/[0.025] p-5 transition hover:-translate-y-0.5 hover:border-neutral-300/25 hover:bg-neutral-400/[0.04]">
              <span className="grid h-10 w-10 place-items-center rounded-xl border border-neutral-300/15 bg-neutral-400/[0.07] text-neutral-300"><Icon className="h-[18px] w-[18px]" /></span>
              ...
              <span className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold text-neutral-300/75 group-hover:text-neutral-200">Open evidence <ArrowUpRight className="h-3.5 w-3.5" /></span>
            </a>
          ))}
        </div>
      </motion.section>
      ...
      <motion.section {...fade(0.19)} className="mt-12 grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
        <div className="rounded-2xl border border-neutral-300/15 bg-[linear-gradient(145deg,rgba(255,255,255,.07),rgba(255,255,255,.018))] p-5 sm:p-7">
          <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Dependency gate</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-xl border border-white/[0.07] bg-black/15 p-4">
                <strong className="text-2xl font-black text-neutral-200">{value}</strong>
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-[#070b09] p-5 sm:p-7">
          <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Source to chain</p>
      ...
      <motion.section {...fade(0.2)} className="mt-12 rounded-3xl border border-white/[0.08] bg-white/[0.02] p-5 sm:p-8">
        <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr] lg:items-start">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Contract surface</p>
            ...
            <a href={`${REPO}/blob/main/docs/security-threat-model.md#contract-surface-review`} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-neutral-300 hover:text-neutral-200">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-xl border border-white/[0.07] bg-black/15 p-4">
                <strong className="text-[10px] tracking-[0.15em] text-neutral-300/75">{label}</strong>
      ...
      <motion.section {...fade(0.21)} className="mt-12 overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.02] p-5 sm:p-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">SDK perimeter</p>
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {clientPackages.map(([name, role], index) => (
            <a key={name} href={`https://www.npmjs.com/package/${name}`} target="_blank" rel="noreferrer" className="group relative overflow-hidden rounded-2xl border border-white/[0.07] bg-black/15 p-4 transition hover:-translate-y-0.5 hover:border-neutral-300/25">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-neutral-300/50 to-transparent opacity-0 transition group-hover:opacity-100" />
              <span className="text-[10px] font-semibold tracking-[0.15em] text-white/25">PACKAGE {String(index + 1).padStart(2, "0")}</span>
              <strong className="mt-3 block break-words font-mono text-xs text-neutral-200/85">{name}</strong>
              <p className="mt-3 text-xs leading-relaxed text-white/40">{role}</p>
              <div className="mt-4 flex items-center gap-1.5 text-[10px] font-semibold text-neutral-300/55">Inspect package <ArrowUpRight className="h-3 w-3" /></div>
            </a>
          ))}
        </div>
      </motion.section>
      ...
      <section className="mt-12 grid gap-6 lg:grid-cols-[1fr_.88fr]">
        <motion.div {...fade(0.2)} className="rounded-2xl border border-white/[0.08] bg-[#070b09] p-5 sm:p-6">
          <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.17em] text-neutral-300/70">Reproduce locally</p><h2 className="mt-2 text-xl font-bold text-white">One command. The same repository gate.</h2></div><Code2 className="h-6 w-6 text-neutral-300/60" /></div>
          <div className="mt-5 flex items-start gap-3 rounded-xl border border-white/[0.08] bg-black/25 p-4">
            <code className="min-w-0 flex-1 overflow-auto whitespace-pre-wrap break-all text-xs leading-relaxed text-neutral-100/70">{VERIFY_COMMAND}</code>
            <button type="button" onClick={copyCommand} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-neutral-300/15 bg-neutral-400/[0.06] text-neutral-300 transition hover:border-neutral-300/30" aria-label="Copy verification command" title="Copy verification command">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</button>
          </div>
        ...
        <motion.div {...fade(0.24)} className="rounded-2xl border border-neutral-300/15 bg-neutral-400/[0.045] p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-3">
            {[
              ...
              <a key={name} href={`https://stellar.expert/explorer/public/contract/${id}`} target="_blank" rel="noreferrer" className="rounded-xl border border-white/[0.07] bg-black/15 p-4 transition hover:border-neutral-300/25">
                <div className="flex items-center justify-between gap-3"><strong className="text-sm text-neutral-100">{name}</strong><ArrowUpRight className="h-4 w-4 text-neutral-300" /></div>
