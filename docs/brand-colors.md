...
The homepage's consumer entry and authorization step are red; developer links, the Ackrate mark and protocol foundation are green. Wallet primary actions and the current step are red. Completed checks, ready configuration and settlement evidence use green while preserving labels/checkmarks. Unknown network/loading states remain neutral. Destructive actions retain explicit wording and an outlined treatment distinct from ordinary continuation.
