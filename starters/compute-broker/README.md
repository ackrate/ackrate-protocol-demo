# Verifiable Compute Broker

**Buy a bounded cryptographic workload and verify the returned digest independently on the consumer side.**

This starter protects `GET /compute/sha256-chain/:tier/:seedId` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Paid resource: `GET /compute/sha256-chain/:tier/:seedId`
- Price policy: exact decimal amounts declared by the scenario
- Safety or recovery check: `compute-tier-over-ceiling`
- Expected outcome: The deterministic hash function rejects an iteration count above 512 before hashing begins.
- Fixtures: Two fixed seeds, two work tiers, one checked deterministic digest prefix, and a maximum iteration count.

Apply fixed work tiers and iteration ceilings, execute a deterministic hash chain, and require the consumer to recompute the expected digest.
...
Catalog identity: `compute-broker` · fixture policy: `deterministic-and-clearly-labeled`.
