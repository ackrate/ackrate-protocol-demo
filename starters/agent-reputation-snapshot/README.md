...
**Buy a time-bounded reputation snapshot derived from signed execution events without treating a score as payment authority.**

This starter protects `GET /agents/:agentAddress/reputation` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Paid resource: `GET /agents/:agentAddress/reputation`
- Price policy: exact decimal amounts declared by the scenario
- Safety or recovery check: `untrusted-reputation-issuer`
- Expected outcome: Events signed by an issuer outside the configured trust set are excluded from the reputation calculation.
...
Verify an issuer allowlist, exclude stale events, weight completed and failed outcomes, and return evidence references beside the score.
...
Catalog identity: `agent-reputation-snapshot` · fixture policy: `deterministic-and-clearly-labeled`.
