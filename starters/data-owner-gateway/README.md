...
**The live fixture merchant serves one owner-alpha dataset endpoint and receives payment when the agent unlocks that dataset.**

This starter protects `GET /datasets/:ownerId/:datasetId` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Paid resource: `GET /datasets/:ownerId/:datasetId`
- Price policy: exact decimal amounts declared by the scenario
- Safety or recovery check: `dataset-owner-mismatch`
- Expected outcome: A dataset mapped to another owner is rejected by the endpoint's owner mapping before a payment challenge.
- Fixtures: Two owner-specific dataset fixtures with stable hashes, observed-at timestamps, and permitted-use metadata.

Resolve an owner-scoped dataset fixture, return its observed-at and permitted-use metadata, and verify the content hash without claiming revenue splits.
...
Catalog identity: `data-owner-gateway` · fixture policy: `deterministic-and-clearly-labeled`.
