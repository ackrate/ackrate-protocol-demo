...
This starter protects `GET /snapshots/:pageId` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Safety or recovery check: `snapshot-resource-rebinding`
- Expected outcome: Payment for one page cannot unlock a different page because the proof is bound to the exact requested resource.
