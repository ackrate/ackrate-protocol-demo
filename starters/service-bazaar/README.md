...
This starter protects `GET /services/:serviceId/results/:fixtureId` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Safety or recovery check: `listing-merchant-mismatch`
- Expected outcome: A wrong-merchant listing is excluded even though its declared price, quality, and latency signals would otherwise rank first.
