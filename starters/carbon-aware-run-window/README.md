...
This starter protects `GET /schedules/:jobId/window` with a request-bound payment on Stellar testnet. The app asks; the MandateRegistry contract decides whether money moves.
...
- Safety or recovery check: `forecast-expired`
- Expected outcome: A forecast older than the scheduling freshness limit is rejected before a run window is selected.
