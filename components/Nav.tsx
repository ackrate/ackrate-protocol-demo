...
export default function Nav() {
  ...
  return <>
    <nav aria-label="Main navigation" className="fixed inset-x-0 top-0 z-50 border-b border-[#1f1f1f] bg-black"
      ...
      <div className="flex h-16 items-center justify-between gap-1 px-2 sm:gap-4 sm:px-6">
        ...
        <div className="flex items-center sm:gap-1">
          ...
          <details key={path} className="relative" onKeyDown={(event) => {
            if (event.key === "Escape") {
              event.currentTarget.open = false;
              event.currentTarget.querySelector("summary")?.focus();
            }
          }}>
            <summary className={`flex min-h-11 cursor-pointer items-center rounded px-1.5 text-xs sm:px-2 sm:text-sm ${guides.some((guide) => path === guide.href) ? "bg-white/10 text-white" : "text-white/60"}`}>Guides</summary>
