...
export default function CliMainnetTestPanel() {
  ...
  const paymentReceipts = presentation.receipts.filter((receipt) => receipt.payment);
  const success = state === "succeeded" && run?.state === "succeeded" && new Set(paymentReceipts.map((receipt) => receipt.hash)).size === 3 && !status?.error && !run.error;
  const evidenceMissing = state === "succeeded" && !success;
  ...
  return (
    <div className="space-y-6">
      ...
      {(status?.configured || run || unavailable) && <details className="rounded-xl border border-zinc-800 bg-zinc-950/60">
        <summary className="cursor-pointer p-4 text-sm font-medium text-zinc-400">Previous team-funded test · {success ? "passed" : evidenceMissing ? "evidence unavailable" : "recorded evidence"}</summary>
    <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 text-zinc-100" aria-label="Mainnet CLI test status">
      <div className="border-b border-zinc-800 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span role="status" className={`rounded-full border px-3 py-1 text-xs ${success ? "border-neutral-500/30 text-neutral-300" : "border-zinc-700 text-zinc-300"}`}>{evidenceMissing ? "evidence unavailable" : state}</span>
        </div>
      ...
      <div className="space-y-4 p-5">
        ...
        {evidenceMissing && <p role="alert" className="text-sm text-zinc-300">The saved job reports completion, but its run evidence is incomplete or could not be loaded. Receipts and delivery cannot be verified here.</p>}
        {success && <div className="rounded-lg border border-neutral-500/20 bg-neutral-500/5 p-4"><h3 className="font-medium text-neutral-300">Payment test passed</h3><p className="mt-2 text-sm leading-relaxed text-zinc-300">Three research sources delivered after three 0.01-USDC payments. The contract blocked purchase four because the 0.03-USDC budget was exhausted.</p><p className="mt-2 text-xs text-zinc-400">This is the completed run. Viewing receipts or downloading evidence does not spend again.</p></div>}
        ...
        <button type="button" disabled={!run?.logs?.trim()} onClick={download} className="rounded-lg bg-zinc-100 px-4 py-2 text-sm font-medium text-black disabled:opacity-40">Download test evidence</button>
      </div>
    </section>
      </details>}
    </div>
