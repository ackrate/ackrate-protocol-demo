...
export default function CliMainnetTestPanel() {
  ...
  const success = state === "succeeded" && run?.state === "succeeded" && Boolean(run.logs?.trim()) && !status?.error && !run.error;
  ...
  return (
    ...
    <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 text-zinc-100" aria-label="Mainnet CLI test status">
      ...
      <div className="space-y-4 p-5">
        ...
        {evidenceMissing && <p role="alert" className="text-sm text-zinc-300">The saved job reports completion, but its run evidence could not be loaded. Receipts and delivery cannot be verified here.</p>}
        ...
        <button type="button" disabled={!run || evidenceMissing} onClick={download} className="rounded-lg bg-zinc-100 px-4 py-2 text-sm font-medium text-black disabled:opacity-40">Download test evidence</button>
      </div>
    </section>
      </details>}
    </div>
